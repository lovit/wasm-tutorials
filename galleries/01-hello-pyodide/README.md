...
WASM 의 경계로 왜 숫자만 오가는지, 여기서 받은 `.wasm` 이 메모리를 얼마로 잡고 뜨는지는 [01. WebAssembly 는 어떻게 도는가](../../docs/tutorials/01-how-wasm-works.md)에 있다. `sys.platform` 이 왜 `emscripten` 인지는 [02. CPython 을 브라우저로 옮기기](../../docs/tutorials/02-emscripten-and-cpython.md)에서 다룬다.
