...
| 방법          | 걸린 시간 | 결과 합 |
| ------------- | --------- | ------- |
| 파이썬 리스트 | 143 ms    | -681.76 |
| numpy 배열    | 16 ms     | -681.76 |

9배쯤이다. 그런데 **버튼을 다시 누르면 숫자가 달라진다.**
...
리스트 쪽은 그대로인데 numpy 쪽이 18 ms 에서 9~10 ms 로 빨라진다. 첫 호출에서 치르는 몫이 있다는 뜻이다. 그러니 화면에 처음 뜨는 9배가 numpy 의 실력은 아니다. 눌러 봐야 안다.
