...
![예제 화면. 위쪽에 준비됐다는 안내가 있고, 그 아래 print 여러 줄·stderr 로 쓰기·오류 내기·input 쓰기 네 개의 예시 버튼이 있다. 파이썬 코드 입력칸에 sys 와 warnings 를 쓰는 코드가 들어 있고, 그 아래 미리 넣어 둘 입력칸은 비어 있다. 실행 버튼 옆에 코드에 파일 이름 붙이기와 트레이스백에서 내부 프레임 감추기 체크박스가 둘 다 켜져 있다. 아래쪽 어디로 갔나 절에는 stdout 칸에 "이건 stdout 으로 갑니다" 한 줄이, 빨간 테두리의 stderr 칸에 "이건 stderr 로 갑니다" 와 user_code.py:9 UserWarning 두 줄이 나뉘어 들어 있다. 맨 아래 돌려준 값은 (없음) 이다.](screenshot.png)
...
`textContent +=` 로 붙이지 않는 이유가 있다. 그렇게 쓰면 줄마다 문자열 전체를 새로 만들어서 출력이 길어질수록 제곱으로 느려진다. 2만 줄에 0.6초, 20만 줄이면 17초가 넘게 걸리고 그동안 페이지가 멈춘다. 텍스트 노드를 덧붙이면 같은 결과가 선형으로 끝난다.
...
남은 조각은 실행이 끝나도 사라지지 않는다. 그대로 두면 다음 실행 출력 앞에 유령처럼 붙는다. `setStdout` 을 다시 걸어도, `sys.stdout.flush()` 를 불러도 비워지지 않는다. 개행을 하나 찍어 주는 것이 유일한 방법이라 이 예제는 실행을 시작할 때 그렇게 한다.

```js
flushLeftover(pyodide);
```

지우기 전에 부르는 것이 요점이다. 흘러나온 조각이 곧바로 지워지므로 이번 출력이 깨끗하다.
...
줄 번호만 있던 것이 실제 소스와 캐럿까지 붙는다. 이름을 주면 Pyodide 가 그 이름으로 소스를 `linecache` 에 등록해 두기 때문이다. 트레이스백은 거기서 줄을 꺼내 온다.

진짜 파일이 생기는 것은 아니다. `os.path.exists("user_code.py")` 는 여전히 `False` 인데 `"user_code.py" in linecache.cache` 는 `True` 다. 그리고 꺾쇠로 감싼 이름(`<exec>` 같은)을 주면 Pyodide 가 등록을 건너뛴다. 그래서 파일을 만들어 둔다고 해결되지 않고, 이름을 주는 것으로 해결된다.

이름을 하나 주는 것만으로 사용자가 읽을 수 있는 오류가 된다. 값이 큰 데 비해 손이 거의 안 든다.

경고에도 같이 적용된다. 화면의 stderr 칸에 `user_code.py:9: UserWarning` 이라고 나오는 것이 그 결과다.

예시 코드에 `warnings.simplefilter("always")` 가 들어 있는 이유가 있다. 파이썬은 같은 자리의 경고를 한 번만 찍는다. 그 기록이 전역에 남으므로, 이 줄이 없으면 두 번째 실행부터 경고가 조용히 사라진다. 체크박스를 껐다 켜며 비교하려는데 경고가 안 보이면 그것 때문이다.
...
프레임 한 줄만 지우면 그 아래 딸린 소스 줄과 캐럿이 남아 떠돈다. 그래서 상태를 하나 들고 딸린 줄까지 함께 버린다.

들여쓰기를 보는 것이 중요하다. "다음 프레임이 나올 때까지 버린다" 로 만들면, 마지막 프레임이 내부 프레임인 경우에 그 뒤의 `XxxError: 메시지` 줄까지 삼킨다. 소스에 널 바이트를 넣어 보면 실제로 그렇게 된다. 컴파일이 Pyodide 내부에서 실패해서 사용자 프레임이 하나도 없기 때문이다. 오류를 읽으라고 만든 화면에서 오류가 사라지는 셈이다.

`raise ... from ...` 처럼 예외가 연쇄되면 트레이스백이 두 덩이로 오고 사이에 `The above exception was the direct cause of the following exception:` 이 낀다. 들여쓰기 규칙 덕분에 그 줄도 그대로 남는다.
...
- `import sys; sys.stdout.write("x\n")` 를 돌려 보자. `print` 를 안 써도 같은 자리로 온다. 스트림 자체를 바꾼 것이기 때문이다. 개행을 빼면 앞의 실험과 똑같이 아무것도 안 보인다
...
| 경고가 처음 한 번만 나온다 | 파이썬이 같은 자리의 경고를 한 번만 찍는다. `warnings.simplefilter("always")` 를 넣자 |
| 앞선 실행의 글자가 이번 출력 앞에 붙는다 | 개행 없이 끝난 조각이 버퍼에 남아 있었다. 실행 전에 개행을 하나 찍어 떨군다 |

## 이 코드는 페이지 전부를 만질 수 있다

입력칸에 이런 것을 넣어 보면 바로 안다.

```python
import js

js.document.title = "가로챘습니다"
```

제목이 바뀐다. `runPython` 에 들어간 파이썬은 `import js` 로 이 페이지의 DOM, 쿠키, 네트워크에 전부 닿는다. 파이썬이 WebAssembly 샌드박스 안에 있는 것은 맞지만, 그 샌드박스는 파이썬을 브라우저에서 가두는 것이지 이 페이지에서 가두는 것이 아니다.

지금은 자기가 친 코드를 자기가 돌리는 것이라 문제가 없다. 남이 준 코드를 넣는 순간 이야기가 달라진다. URL 파라미터로 코드를 받거나, 공유 링크로 남의 코드를 실행하게 만들면 그 페이지를 통째로 내주는 것과 같다. 격리하려면 [17. 워커로 옮기기](../17-web-worker/)처럼 DOM 이 없는 곳으로 옮겨야 한다.
